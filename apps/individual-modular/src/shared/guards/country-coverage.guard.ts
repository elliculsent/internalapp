...
import {COVERAGE_INFORMATION, COVERAGE_INFORMATION_LINK, PERSONALISATION_LINK} from '@uex/uex-constants';
import {State} from '@individual-modular/reducers';
import * as selectors from '@individual-modular/selectors';

@Injectable()
export class CountryCoverageGuard implements CanActivate {
    private isValid: boolean;
    private selectedLanguageCountry: string;

    constructor(private router: Router, private store: Store<State>) {
        this.store.pipe(select(selectors.isPersonalisationSubFlowValid, COVERAGE_INFORMATION)).subscribe((data) => {
            this.isValid = data;
        });
